...
- **Binding de parches**: los nombres de parámetro del prefix/postfix deben coincidir **exactos** con los del método original (`piece`, `granted`, `clickHandler`), y los especiales (`__instance`, `__result`, `__state`) bien escritos. Un typo no se detecta en compilación. Verificá cada firma contra `<mod>/docs/api-*.md` (hoy `QuickStash/docs/api-1.0.7.md` y `CrewRow/docs/api-barcos-1.0.7.md`) o decompilando.
...
6. **Tu mensaje final ES el entregable** — reporte completo, no resumen. Si podés escribir sin bloqueo, guardá además `<mod>/docs/code-review-YYYY-MM-DD.md`.
